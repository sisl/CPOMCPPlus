...
@with_kw mutable struct CPOMCPSolver <: AbstractCPOMCPSolver
    ...
    plus_flag::Bool = false ##adding switch for plus
end
...
mutable struct CPOMCPPlanner{P, SE, RNG} <: AbstractCPOMCPPlanner{P,SE,RNG}
    ...
    plus_flag::Bool ##adding switch for plus
end
...
function CPOMCPPlanner(solver::CPOMCPSolver, pomdp::CPOMDP)
    ...
    # if solver.plus_flag
    #     println(solver.plus_flag)
    # end

    return CPOMCPPlanner(solver, pomdp, se, solver.rng, 
        costs_limit(pomdp), Int[], nothing, nothing, nothing, solver.plus_flag)
end
...
struct CPOMCPTree{A,O} <: AbstractCPOMCPTree
    ...
    lambda::Vector{Vector{Float64}}      # lambda estimates for an action node
    #when pushed will be the size of o and not a takes care of self
...
function CPOMCPTree(pomdp::CPOMDP, b, sz::Int=1000; init_λ::Union{Nothing,Vector{Float64}} = nothing) #can pass this in directly?n_costs(pomdp))
    ...
    if init_λ === nothing
        #print(init_λ)
        init_λ = zeros(Float64, n_costs(pomdp)) # start unconstrained
        #print(init_λ)
    end

    tree = CPOMCPTree{A,O}(sizehint!(Int[0], sz),
                          sizehint!(Vector{Int}[collect(1:length(acts))], sz),
                          sizehint!(Array{O}(undef, 1), sz),
                          sizehint!(Dict{Tuple{Int,O},Int}(), sz),
                          sizehint!([init_λ], sz),
                          #sizehint!(repeat([zeros(Float64,cons)], 1), sz), # lambda
                          #repeat for all numbers of action, length of observations is 1
...
function insert_obs_node!(t::CPOMCPTree, p::CPOMCPPlanner, ha::Int, sp, o, h::Int) #add h so func can be called with CPOMCPTree
    pomdp = p.problem
    acts = actions(pomdp, LeafNodeBelief(tuple((a=t.a_labels[ha], o=o)), sp))
    push!(t.total_n, 0)
    push!(t.children, sizehint!(Int[], length(acts)))
    push!(t.o_labels, o)
    p.plus_flag && push!(t.lambda, t.lambda[h]) #pushing lambda with parent value
    hao = length(t.total_n)
    t.o_lookup[(ha, o)] = hao
    for a in acts
        n = insert_action_node!(t, p, hao, a)
        push!(t.children[hao], n)
    end
    return hao
end

function insert_action_node!(t::CPOMCPTree, p::CPOMCPPlanner, h::Int, a)
    push!(t.n, 0)
    push!(t.v, 0.0)
    push!(t.a_labels, a)
    push!(t.cv, zeros(Float64, t.n_costs))
    p.plus_flag && push!(t.top_level_costs, zeros(Float64, t.n_costs))
    return length(t.n)
end
...
function CPOMCPDPWPlanner(solver::CPOMCPDPWSolver, pomdp::CPOMDP)
    ...
    return CPOMCPDPWPlanner(solver, pomdp, se, solver.rng, solver.next_action,
        costs_limit(pomdp), Int[], nothing, nothing, nothing)
end
