### Discrete Light Dark New
## Same as clightdark, but with a discrete observation space
...
# Creates a categorical distribution centered around x, with a width of 2 * n + 1 
# α is the total probability distribution assigned to the surrounding values
function POMDPs.observation(p::LightDarkNew, sp::LightDark1DState)
    α::Real=0.8
    n::Real=5
    x::Int = trunc(sp.y)
    values = range(x - n, x + n, step = 1.0) # [O(i) for i in ]
    decay_rate::Real = 4 / 5.
    probs = [α/2 * (1 - decay_rate) * ((decay_rate) ^ abs(x - i)) for i in values]
    probs[ceil(Int64, n/2)] = 1-α
    return POMDPModelTools.SparseCat(values, probs)
end
