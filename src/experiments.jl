...
function run_cpomdp_simulation(cpomdp::CPOMDP, solver::Solver, 
    ...
    for (s, a, o, r, c, sp, b, ai) in stepthrough(cpomdp, planner, bu, "s,a,o,r,c,sp,b,action_info", max_steps=max_steps)
        ...
        #save only the first action taken
        if γ == 1
            ...
            #how to add visits to each action?
            actions = [0, 1, 2]
        ...
        if track_history
            push!(hist, (;s, a, o, r, c, sp, b, 
                ...
                v_taken = :v_taken in keys(ai) ? ai[:v_taken] : nothing, #these are the simulation, not the step
                cv_taken = :cv_taken in keys(ai) ? ai[:cv_taken] : nothing,
                ))
        end
    end

    return hist, R, C, RC, first_vals
end
